 ...
 * 
 * -p pollRecords
 * The maximum number of records to poll. Optional, defaults to 500.
 * 
 * -i intervalSeconds
 * The maximum time to allow for processing records before polling. Optional, defaults to 10.
 * 
 * -d processingSeconds
 * The (simulated) time required to process each message. Optional, defaults to 1.
 */
public class ConsumerCLI {
	...
	private ConsumerCLI(String apikey, String bootstrapServers, String topicName, String consumerGroup, int pollRecords, int intervalSeconds, int processingSeconds) {
		this.apikey = apikey;
	...
	static ConsumerCLI parse(String[] args) { 
		...
		options.addOption(
				Option.builder("p")
					.hasArg(true)
					.argName("pollRecords")
					.desc("(optional, default 500) maximum number of records to poll")
					.required(false)
					.build());
		options.addOption(
				Option.builder("i")
					.hasArg(true)
					.argName("intervalSeconds")
					.desc("(optional, default 10) time allowed for processing before next poll")
					.required(false)
					.build());
		options.addOption(
				Option.builder("d")
					.hasArg(true)
					.argName("processingSeconds")
					.desc("(optional, default 1) time required to process a record")
					.required(false)
					.build());
		...
		try {
			...
			printHelpAndExit("Invalid pollRecords value: " + e.toString(), options);
		}
		int intervalSeconds = 10;
		try {
			intervalSeconds = Integer.parseInt(cmd.getOptionValue("i", "10"));
			if (intervalSeconds <= 0) {
				printHelpAndExit("Invalid intervalSeconds value, must be greater than 0", options);
			}
		} catch (Exception e) {
			printHelpAndExit("Invalid intervalSeconds value: " + e.toString(), options);
		}
		int processingSeconds = 10;
		try {
			processingSeconds = Integer.parseInt(cmd.getOptionValue("d", "10"));
			if (processingSeconds <= 0) {
				printHelpAndExit("Invalid processingSeconds value, must be greater than 0", options);
			}
		} catch (Exception e) {
			printHelpAndExit("Invalid processingSeconds value: " + e.toString(), options);
		}
		
		return new ConsumerCLI(apikey, bootstrapServers, topicName, consumerGroup, pollRecords, intervalSeconds, processingSeconds);
	}
