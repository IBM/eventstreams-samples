        ...
        System.out.println("\n** Sending messages\n");
     	for (int i = 0; !shutdown && i < cliArgs.messageCount; i++) {
	        ...
	        try {
	        	...
	        	System.out.printf("** Successfully sent message %d, topicPartition=%s-%d offset %d\n", 
	        			messageNum, metadata.topic(), metadata.partition(), metadata.offset());
		        if (cliArgs.messageDelay > 0) {
		        	Thread.sleep(cliArgs.messageDelay * 1000L);
		        }
	        } catch (Exception e) {
